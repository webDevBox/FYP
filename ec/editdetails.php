...
// if everything is ok, try to upload file
    ...
    $sql ="select username from tbl_hoc where id=".$_SESSION['id'];
...
$uss = $row['username'];
    ...
    $imgsrc = $target_dir.$_SESSION['id'].$uss.'.'.$imageFileType;
		if (move_uploaded_file($_FILES["img"]["tmp_name"], $imgsrc)) {
			$sql = "UPDATE `tbl_hoc` SET `img`='".$imgsrc."' WHERE id=".$_SESSION['id'];
           if($con->query($sql)){
               $_SESSION['img']=$imgsrc;
...
include 'connection.php';
	...
	$email = $_POST['em'];
	$gender = $_POST['gender'];
        $sql = "UPDATE `tbl_ec` set `fname`='".$fname."',`lname`='".$lname."',`university`='".$university."',`country`='".$country."',`username`='".$username."',`pass`='".$pass."',`email`='".$email."',`gender`='".$gender."',`qualifications`='".$qual."' WHERE id=".$_SESSION['id'];
...
<div class="col-sm-8">
  <?php
  $sql ="SELECT * FROM `tbl_hoc`  where id=".$_SESSION['id'];
 $result = $con->query($sql);
